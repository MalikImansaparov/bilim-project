...
export const Enroll = () => {
  ...
  const { isLoading  } = useFetch(base + eventsUrl + '/events/src/bilim/');
  const [response, setResponse] = useState([])
  const {t,i18n} = useTranslation()

  const getSliders = async () => {
    const res = await axios.get(base + eventsUrl + '/events/src/bilim/')
  ...
  return (
    <div className="w-[100%] max-h-[320px] m-auto relative mb-[100px] z-10">
