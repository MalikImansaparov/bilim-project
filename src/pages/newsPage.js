...
const NewsPage = () => {
  const { isLoading } = useFetch(base + newsUrl + '/news/src/bilim/');
  ...
  const getData = async () => {
    const res = await fetch(`${base}${newsUrl}/news/src/bilim/`);
  ...
  const paginateData = async (count) => {
    const res = await fetch(`${base}${newsUrl}/news/src/bilim/?page=${count}`);
    const data = await res.json();
    return data;
  };
