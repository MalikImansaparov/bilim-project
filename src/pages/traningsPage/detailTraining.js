...
const DetailTraining = () => {
  window.scroll(0, 0);
  const { id } = useParams();
  const { isLoading, response } = useFetch(api + `/training/${id}/`);
          ...
          {i18n.language === 'ky' && (
            <div className="wrapper">
              ...
              <div className="mb-8 relative">
                ...
                <div className="flex flex-wrap my-4 md:max-w-[500px] justify-between 2md:text-[14px]">
                  ...
                  <div className="flex 1xs:mt-1">
                    ...
                    {i18n.language === 'ky' && (
                      <p className="ml-1 self-start flex items-center">
                        <span className="font-bold mr-0.5">{t('where')} </span>{' '}
                        {response.place_ky}
                      </p>
                    )}
                    {i18n.language === 'ru' && (
                      <p className="ml-1 flex items-center">
                        <span className="font-bol mr-0.5d">{t('where')} </span>{' '}
                        {response.place_ru}
                      </p>
                    )}
                    {i18n.language === 'en' && (
